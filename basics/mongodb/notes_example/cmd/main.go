...
import (
	"fmt"
	"mongodb/notes_example/model"
	"mongodb/notes_example/repository"
)

func main() {
	noteRepository, err := repository.NewNoteRepository()
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	if id, err := noteRepository.Create(note); err == nil {
		fmt.Println("New record has been created with id:", id)
		if qNote, err := noteRepository.GetById(id); err != nil {
			fmt.Println(err)
		} else {
	...
	if _, err := noteRepository.Create(note); err != nil {
		fmt.Println("error while creating new Note:", err)
	}
	...
	id, err := noteRepository.Create(note1)
	if err == nil {
		fmt.Println("New record has been created with id:", id)
	}
	// update
	note1.Title = "mongodb database"
	if err := noteRepository.Update(id, note1); err != nil {
		fmt.Println("error while updating Note:", err)
	} else {
		fmt.Println("Updated Note")
	}
	// get all
	if notes, err := noteRepository.GetAll(); err == nil {
	...
	if err := noteRepository.Delete(id); err != nil {
